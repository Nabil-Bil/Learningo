        ...
        <div class="bg-white shadow-xl sm:rounded-lg p-4 pb-10 border-2 border-gray-600 break-words">
            <div class="divide-y">
                <div>
                        ...
                        <div class="flex">
                            ...
                            <div class="flex mx-3 flex-col">
                                ...
                                <span class="text-xs text-gray-400">
                                    @if (
                                    Carbon\Carbon::parse(today())->isoFormat('D/M/Y')==Carbon\Carbon::parse($post->created_at)->isoFormat('D/M/Y'))
                    ...
                    <div class="flex p-4 border m-2 items-center cursor-pointer bg-gray-100" wire:click="download({{ $file }})">
                        ...
                        <p class="text-lg font-semibold mx-4">{{ $file->file_name }}</p>
                    </div>
                    @endforeach
                ...
                <div>
                    @error('add_comment') <span class="error text-xl font-bold text-red-500 mb-5">{{ $message }}</span>
                    @enderror
                    <form class="flex items-center px-5 my-3" wire:submit.prevent='submit'>
                        <img class="h-8 w-8 rounded-full object-cover mr-10 "
            ...
            <div x-data="{open : false}">
                <div class="text-center">
                    <button @click="open = !open" class="border-none underline font-bold text-xl ">Show All
                        Comments</button>
                </div>
